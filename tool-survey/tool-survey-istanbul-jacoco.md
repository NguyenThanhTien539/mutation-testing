...
### 2.11 Tiềm năng demo
Infection Playground - https://infection-php.dev/

Đây là môi trường thực hiện Mutation Testing với Infection trực tiếp trên trình duyệt.

Đã có sẵn mã demo, người dùng có thể viết trước mã nguồn và các bài test, sau đó nhấn "Run" để xem kết quả kiểm thử đột biến.
...
- Đạt mutation score có thể không cao bằng 1 số framework khác cho Java (như MUJAVA).

### 3.9 Chi phí/Giấy phép
- Chi phí: Miễn phí
- Giấy phép: CC0 1.0 Universal license.

### 3.10 Hỗ trợ AI, nếu có
Major Mutation Framework là một công cụ kiểm thử truyền thống nên không được tích hợp AI ở mức cơ bản.
...
- Tài liệu đây đủ từ tác giả: [https://mutation-testing.org/index.html](https://mutation-testing.org/index.html)
- Analysing and Comparing the Effectiveness of
Mutation Testing Tools: A Manual Study: [https://orbilu.uni.lu/bitstream/10993/28929/1/SCAM16A.pdf](https://orbilu.uni.lu/bitstream/10993/28929/1/SCAM16A.pdf)

## 4. Ghi chú tổng kết

Cả **Infection PHP (PHP)** và **Major (Java)** đều là những công cụ nền tảng hỗ trợ đắc lực cho việc nghiên cứu và đánh giá chuyên sâu trong seminar Mutation Testing & Test Effectiveness. Mặc dù việc đo lường mức độ bao phủ của bộ kiểm thử (Code Coverage) là cơ sở ban đầu, hai công cụ kiểm thử đột biến này tập trung vào khía cạnh nâng cao hơn là kiểm tra hiệu quả thực sự của các bài test.

- **Đánh giá bao phủ cơ bản (Code Coverage):** Đóng vai trò làm tiền đề, đảm bảo mã nguồn đã được chạy qua trước khi thực hiện đột biến.
- **Mutation Testing (Infection PHP, Major):** Đi sâu vào việc bắt lỗi logic bằng cách xác định xem bài kiểm thử có bị lỗi (bắt được lỗi) khi mã nguồn bị thay đổi hay không. Cả hai công cụ đều thực hiện điều này bằng cách nhúng các đột biến (mutants) vào Cây cú pháp trừu tượng (AST) và đo lường tỷ lệ các đột biến bị tiêu diệt (killed), thoát (escaped) hay còn sống (live mutants).

Trong buổi seminar, hai công cụ này có thể được sử dụng làm trung tâm để xây dựng một kịch bản thuyết trình và demo hoàn hảo cho hai hệ sinh thái ngôn ngữ khác nhau:

1. **Thiết lập cơ sở:** Sử dụng các công cụ Code Coverage thông thường để cho thấy bộ kiểm thử có thể dễ dàng đạt độ bao phủ cao, tạo cơ sở trước khi áp dụng kỹ thuật nâng cao. (Lưu ý, với Infection PHP, CodeCoverage bắt buộc phải được chạy trước để báo cáo đột biến).
2. **Tiến hành đột biến:** Chạy Infection PHP (đối với mã PHP) hoặc Major (đối với mã Java) để cấu hình và nhúng các toán tử đột biến (mutator) trực tiếp vào mã nguồn/AST.
3. **Phân tích điểm yếu của bộ test:** Thực thi bộ kiểm thử trên các đột biến đã tạo. Thu thập kết quả và báo cáo các đột biến còn sống (escaped/live mutants) để chứng minh rằng, mặc dù coverage cao, bộ kiểm thử vẫn bỏ sót những lỗi logic.
4. **Chứng minh sự cải thiện (Test Effectiveness):** Tối ưu hóa các bài test case. Sau đó, chạy lại quá trình phân tích (analyzer) của Major hoặc Infection để thu thập báo cáo cập nhật (ví dụ: file `killed.csv` của Major hoặc ma trận kiểm thử đột biến hoàn chỉnh) nhằm chứng minh bộ test giờ đây đã đạt chất lượng và hiệu quả cao hơn.
