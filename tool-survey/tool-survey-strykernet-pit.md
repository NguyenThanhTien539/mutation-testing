...
Tiềm năng demo cao nếu nhóm có C#/.NET project hoặc có thể tạo sample project nhỏ.

Demo flow đề xuất:

1. Tạo function đơn giản, ví dụ kiểm tra tuổi, discount hoặc permission.
2. Viết test suite còn thiếu boundary case.
3. Chạy `dotnet stryker`.
4. Cho thấy một mutant `survived`.
5. Thêm test case còn thiếu.
6. Chạy lại Stryker.NET.
7. Cho thấy mutant đã bị `killed` và mutation score tăng.

Demo này dễ hiểu vì người xem thấy trực tiếp sự khác biệt giữa code coverage và test effectiveness.
...
Tiềm năng demo rất cao nếu nhóm dùng Java/Maven.

Demo flow đề xuất:

1. Tạo Java method đơn giản, ví dụ tính discount hoặc kiểm tra điều kiện tuổi.
2. Viết JUnit test còn thiếu boundary case.
3. Chạy PIT bằng Maven.
4. Mở HTML report và chỉ ra mutant survived.
5. Thêm JUnit test còn thiếu.
6. Chạy lại PIT.
7. Cho thấy mutant bị killed và mutation score tăng.
...
PIT có thể tạo mutant:

```java
return age > 18;
```

Nếu thêm test `age = 18`, mutant sẽ bị killed.
...
Stryker.NET và PIT đều phục vụ cùng một mục tiêu: đo khả năng phát hiện lỗi của test suite thông qua mutation testing. Điểm khác biệt chính nằm ở ecosystem và implementation strategy.

Stryker.NET phù hợp với .NET/C# project, có trải nghiệm tốt qua `dotnet stryker`, nhiều reporter và hỗ trợ workflow như `since`, baseline, thresholds. PIT phù hợp với Java/JVM project, đặc biệt là Maven, với lợi thế bytecode mutation, coverage-guided execution và hệ thống mutator mature.

Kết luận quan trọng cho seminar:

- Code coverage chỉ cho biết code có được chạy hay không.
- Mutation testing cho biết test có thật sự bắt được lỗi hay không.
- `Killed mutants` cho thấy test suite phát hiện được fault.
- `Survived mutants` chỉ ra missing tests hoặc weak assertions.
- Stryker.NET là lựa chọn tốt cho .NET demo.
- PIT là lựa chọn tốt cho Java demo.
- Cả hai đều nên được dùng có chọn lọc trên critical code hoặc changed code vì mutation testing tốn thời gian hơn normal unit testing.
